namespace MintPlayer.Spark.Abstractions.ClientOperations;

/// <summary>
/// Wire envelope for every action-endpoint response. <see cref="Result"/> holds the
/// primary payload (PO, query result, etc.; null when the action returns no body);
/// <see cref="Operations"/> holds the accumulated side-effects for the frontend
/// to dispatch.
/// </summary>
public sealed class ClientOperationEnvelope
{
    public object? Result { get; init; }
    public required IReadOnlyList<ClientOperation> Operations { get; init; }
}
