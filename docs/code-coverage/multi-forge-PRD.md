...
| **D24** | **Mail bodies are plain strings; no templating.** Spark ships the *wording* (`SparkLinkConfirmationMessage`) because it is part of the security design, and no transport. A mail manager is separate, future work. | **DECIDED** — owner, 2026-09-21: *"For now, the emails can be just plain strings. A mailmanager will be planned for the future."* |
| **D25** | **The provider segment goes only where the id embeds a forge-assigned identifier that is not globally unique.** So: `Repositories`, `Accounts`, `Commits` (and everything nested under a commit id), `PullRequestFeedbacks`. **Not** `GitHubProjects` (already a globally-unique GraphQL node id), **not** `ApiTokens` (Raven id generator; forge ids live in fields, which D6e qualifies instead), and not the Spark-owned collections. | **DECIDED** — M6e. A universal prefix would churn ids that cannot collide, and Raven ids are immutable, so every unnecessary re-key is paid once and never recovered. |
| **D26** | **The re-key runs as an ordinary Spark migration in the startup path**, not as an out-of-band admin command. | **DECIDED** — owner, 2026-09-21: *"shouldn’t the migrations be run automatically when the new application/docker container is deployed?"* Verified against the runner rather than assumed: `RunAtStartup` blocks before the app serves, the applied-marker is written only after `Up` succeeds, and a distributed lock prevents a double-run. ⚠️ Conditions in the plan’s M6 section. |
| **D27** | **The provider always precedes the owner, and on the client it is the outermost segment:** `/{provider}/r/{owner}/{name}`, `/{provider}/a/{login}`, `/badge/{provider}/{owner}/{name}.svg`, `/api/browse/repos/{provider}/{owner}/{name}`. | **DECIDED** — owner, 2026-09-21: *"perhaps if possible we could do /github/r/... and /github/a/..."*. D11 said `/{provider}/{owner}/{name}`, which is ambiguous against the top-level namespace; keeping the `r`/`a` discriminator in second position removes that while leaving the forge as the outermost scope, which is how people read it. ⚠️ Badge URLs land in READMEs and PR comments, so this is effectively permanent. |
