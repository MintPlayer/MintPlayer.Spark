...
| Shutdown unspecified | The feeder **awaits its lane pumps**, and each pump drains the handler work it started | Cancelling a token only *asks* a pump to stop. Without waiting, handlers kept querying a store the host was already disposing — abandoning work mid-message in production, and in tests causing the database deletion to time out against live queries. Both bounded (30s / 15s) so a handler ignoring its token cannot hold shutdown open |
