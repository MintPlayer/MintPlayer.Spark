...
decided or measured on **2026-09-07**, except **C15**, which restates the owner's intent for the two
board-automation flags on **2026-09-08** and supersedes C14's resolution; the reasoning lives in the two PRDs, but the *decisions* and
...
| **C15** | *(C14's own resolution)* that the two flags shipped in #377 mean what the owner asked for — `MoveLinkedIssues` for PR events, and a linked-issue-scoped `AddLinkedIfMissing` for recruitment | **The second flag was built against the wrong reading, and the first was documented against it.** The intent, restated by the owner on 2026-09-08 after reading the Dutch label: the two flags answer **orthogonal** questions. `MoveLinkedIssues` = *which items does the rule move* — the event's subject, or its linked issues **as well** (unchanged in behaviour, but "too", not "instead of"). The second flag = *when an item this rule must move is not on the board, add it or skip it* — and it governs **the event's own subject** (the issue on an issue event, **the PR on a PR event**) as well as any linked issue, hence its name: `AutoAddToBoard`, default **`true`**. What shipped instead scoped recruitment to *linked* issues only and left the subject to hard-coded type-aware policy (issues always added, PRs never), which no rule could override — so "add to board" was un-configurable for the item the event was actually about. Also settled: `MoveLinkedIssues` stays **PR-events-only** (GitHub has no issue→issue closing link; extending it to an issue's linked PRs or to sub-issues was declined), and one flag — not two — governs recruitment everywhere | Spec corrected in `docs/prd/PRD-ProjectBoardAutomation.md` decisions 5–7 and §5a. Wrong-reading sites in code: `ProjectAutomationRecipient.cs` (`addIfMissing: true` on the issue path, `addIfMissing: false` on both PR paths, `rule.AddLinkedIfMissing` only inside `MoveLinkedIssuesAsync`), `EventColumnMapping.cs:122`, `App_Data/Model/EventColumnMapping.json` (+ nl/fr descriptions, whose Dutch text is what surfaced this) |
...
| **C16** | *(implicit in #377)* that giving `MoveLinkedIssues` a C# default of `true` made existing rules move linked issues | **A property initialiser only runs for a newly constructed object.** Every rule saved before #377 has no `MoveLinkedIssues` member, and an absent JSON boolean deserialises to `false` — so the default never applied to them and the linked-issue movement #377 existed to deliver has **never once run** for the board the bug was reported from. #377 shipped the field with no migration. The same trap was about to fire a second time on `AutoAddToBoard`; both are now backfilled by `M_202609081200_RenameAddLinkedIfMissing`, which writes `true` only where the member is absent | Measured against the **development** database 2026-09-08, before migrating: `GitHubProjects/PVT_kwDOAug2bM4AthJv`'s one rule read `{"EventType":"PullRequestMerged","TargetColumnOptionId":"98236657","Enabled":true}` — neither flag present. After startup applied the migration, both read `true`, confirmed in the browser with both boxes ticked. Generalises [[reference-raven-missing-field-default-match]]: it is not only queries that read an absent field as `false`, deserialization does too |

**Standing lesson from C16:** a new bool on an existing embedded collection needs a migration the
moment its default is `true`, and "the C# default covers it" is never a reason to skip one. The
failure is silent in the worst direction — the feature simply does not happen, on exactly the
documents that predate it, while a freshly created one works and makes the field look fine.

**Standing lesson from C15:** a per-rule boolean's name has to say what the *user* is choosing, not what the code does at the one call site that reads it. `AddLinkedIfMissing` described an implementation detail of the linked-issue loop; the setting the user was reaching for was "may this rule put things on my board?". Naming it after the loop is what let a wrong scope survive a PRD, a model file, and three translations — and the translation is where it was finally caught, because writing "gekoppeld issue" out in full made the mismatch impossible to miss. **Read the generated nl/fr descriptions of a new option as a spec review, not as a localisation chore.**
...
| OD2 | Boards unit: top-level list, Account sub-query, or both? | ~~Both~~ → **Account sub-query only** (owner, 2026-09-08). Both shipped, and the top-level `GitHub → Project boards` unit was then removed: a user reaches a board from the account that owns it, which is where they already are when they care about one, and the sub-query scopes the list to that account for free. The top-level unit duplicated it one click from Home with a less useful shape. `GetGitHubProjects` stays declared and granted, so `/query/github-projects` still answers a direct link — it just has no menu entry, and `programUnits.json` no longer has a `GitHub` group at all |
