...
> **The remaining confirmation:** the metadata is registered, but whether Spark's middleware actually *validates* it for a handler that reads the body with `ReadFormAsync` is reasoned, not observed. `SparkMiddleware.cs:184-196` looks up `IAntiforgeryMetadata` and calls `ValidateRequestAsync` independently of model binding, which is why this should work — but it is exactly the kind of "should" that E2E test **A-F1/F2** exists to settle. Do not treat O3 as fully closed until that test passes.
