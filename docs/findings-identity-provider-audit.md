...
- **O12** — `post_logout_redirect_uri` validated against **every** enabled application, so one client's URI is accepted in another's logout. Also loads the whole application collection per logout. **Confirmed still reproducing** on this branch (test **L-G4**).
...
- **O25** — RavenDB string equality is case-insensitive by default; `ClientId` lookups don't use `Exact()`. **Raised in priority by the test-matrix review:** if `client_id=acmeapp` resolves the application registered as `AcmeApp`, that is client impersonation by casing, not a cosmetic lookup quirk. Confirm empirically (test **A-C4**) before deciding severity — the answer depends on RavenDB's default term analysis, so read it off a running query rather than reasoning about it.
