...
- **~~O25~~ — CLOSED (verify with A-C4).** RavenDB compares strings case-insensitively by default and the `ClientId` lookup did not opt out, so `acmeapp` would resolve the application registered as `AcmeApp` — impersonation by casing, on the lookup that decides which client every other check is then applied to. Now `exact: true`. **Still worth running A-C4**: the fix is applied on reasoning about RavenDB's default term analysis, and the test is what turns that into an observation.
