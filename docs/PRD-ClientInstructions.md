# PRD: Client Instructions — Unified Backend Side-Effects Envelope
...
Every ad-hoc addition ("let's add a `navigateTo` field to the Create response") fragments the client-handling code and paints the frontend into a corner where each endpoint needs its own response-parsing logic. A **unified envelope** where the backend accumulates typed instructions and the frontend dispatches them through a single handler makes all of the above one change, not N changes.

The retry-deep-dive agent's conclusion: **subsume retry into the unified envelope**, don't run parallel pipelines. Retry becomes one instruction type among many, and the pattern it pioneered (accumulate on `IManager` during action execution, serialize on response egress) extends naturally to the rest of the list above.

## Goals

1. A single **wire envelope shape** carried on every action-endpoint response (HTTP 2xx and HTTP 449), carrying the primary result plus an ordered list of **client instructions**.
2. A single **backend accessor** on `IManager` (e.g. `manager.Client`) where action code accumulates instructions during execution, same scoping model as `IRetryAccessor`.
3. A single **frontend dispatcher** (a service wired into the existing `SparkService` response pipeline) that iterates received instructions and executes each through a registered handler.
4. Unknown instruction types are **ignored by the client** (no hard failure) — enables adding instruction types without breaking older client builds.
...
6. **Retry subsumed**: existing `manager.Retry.Action(...)` calls continue to work at the developer-API level, but internally they contribute a `retry` instruction to the same envelope and travel via the same serialization path.

## Non-goals

- WebSocket / SignalR / real-time push. Instructions ride on the synchronous HTTP response to the triggering action, not a persistent channel.
- Cross-session broadcasts. An instruction affects *the caller's session only*. "Tell every open browser to refresh" is a different problem.
- Pre-action client state queries. The backend does not inspect or probe client state before emitting an instruction.
...
A single envelope wraps every action-endpoint response. The primary result (the thing the endpoint used to return directly) moves under a `result` field; instructions ride alongside under `instructions`.
...
```jsonc
// HTTP 449 — retry pattern subsumed. Note: retry is just an instruction.
...
### Initial instruction types (v1 set)
...
| `retry` | `step`, `title`, `options`, `defaultOption?`, `persistentObject?`, `message?` | (Existing retry pattern, now an instruction type.) Frontend opens the retry modal. HTTP status is 449 iff this instruction is present. |

**Extensibility contract:** new instruction types require:

1. A new DTO class in `MintPlayer.Spark.Abstractions/ClientInstructions/` inheriting a common `ClientInstruction` base with a discriminator `type` string.
2. A builder method on `IClientAccessor` (e.g. `manager.Client.DisableAction(...)`).
3. A frontend handler registered with the dispatcher (`ClientInstructionDispatcher`).
4. No wire-format rev — unknown instruction types are silently ignored by older frontends. This is **v1's single most important compatibility property**.
...
public interface IClientAccessor
{
    ...
    // accumulates a `retry` instruction on this accessor when Action() is called.
...
public interface IManager
{
    ...
    /// <summary>
    /// Accumulates client-side instructions (navigate, notify, refresh, disable, retry)
...
**Retry subsumption:** `IRetryAccessor.Action(...)` internally calls into `IClientAccessor` to push a `retry` instruction, then throws `SparkRetryActionException` (same as today) to unwind. The exception handler in each endpoint no longer builds the retry-specific JSON — it reads the unified envelope off `IClientAccessor` and serializes that. This means:
...
`IClientAccessor` is `ServiceLifetime.Scoped` — same as `IRetryAccessor`, same as `IManager`. One instance per HTTP request. Instructions accumulate in a private `List<ClientInstruction>` and are drained during response serialization.

### Blocking vs non-blocking instructions

Two intrinsic categories based on what the instruction *does* — not a design choice:

| Category | Semantics | HTTP behavior | Examples |
|---|---|---|---|
| **Non-blocking** | Fire-and-forget. Method accumulates the instruction and returns; the action keeps executing. | Rides out on whatever response eventually goes (2xx on success, 4xx/5xx on error, 449 if a blocking instruction later fires). | `navigate`, `notify`, `refreshAttribute`, `refreshQuery`, `disableAction` |
| **Blocking** | Action cannot proceed without user input. Method accumulates the instruction then **throws** via `SparkClientBlockException` (subclass of today's `SparkRetryActionException`, reused name for backward compat inside the framework). The framework catches, serializes the envelope, returns HTTP 449. | HTTP 449 with the full accumulated envelope (including any non-blocking instructions queued before the throw). | `retry` (currently the only one) |
...
**Key property: non-blocking instructions emitted before a blocking throw are preserved in the 449 response.** So the two `Notify`/`RefreshQuery` calls above are dispatched *before* the retry modal opens — which is what users want. Pre-dialog side-effects fire first, then the dialog asks for input.

**Forward-compat:** adding a new blocking instruction type (hypothetical `"authorize"`, `"stepUpAuth"`, etc.) means a new method on `IClientAccessor` that accumulates + throws. The accumulator path is unchanged; the frontend dispatcher just learns a new instruction type. No reshuffling of the mechanism.

### Frontend dispatcher

`@mintplayer/ng-spark` grows a new service, `SparkClientInstructionDispatcher`, injected at root:

```typescript
@Injectable({ providedIn: 'root' })
export class SparkClientInstructionDispatcher {
  private handlers = new Map<string, ClientInstructionHandler<any>>();

  register<T extends ClientInstruction>(
    type: string,
    handler: ClientInstructionHandler<T>
  ): void { /* ... */ }

  dispatch(instructions: ClientInstruction[]): void {
...
`SparkService` gets a response interceptor (analog of the existing `handleRetryError`) that unwraps `{ result, instructions }`, dispatches the instructions, and returns the unwrapped `result` to the caller so consumer code sees the primary payload exactly as today.
...
Apps can register custom handlers via `SPARK_CLIENT_INSTRUCTION_HANDLERS` provider token for instruction types beyond the v1 set.

## Implementation plan

Single PR. Preview-mode project — breaking changes are fine, no deprecation path, no rolling migration. The original three-phase breakdown (envelope+notify+navigate → refresh+disable → retry subsumption) was defensive phasing for a non-existent backward-compat constraint. Phasing buys almost nothing here: the envelope shape threads through every endpoint exactly once, the dispatcher registration pattern is shared across instruction types, and retry subsumption lands cleanest when the new envelope code is fresh rather than being retrofitted weeks later.

### Scope of the PR

**Server side:**
- **New**: `MintPlayer.Spark.Abstractions/ClientInstructions/` — `ClientInstruction.cs` (base), `NavigateInstruction.cs`, `NotifyInstruction.cs`, `RefreshAttributeInstruction.cs`, `RefreshQueryInstruction.cs`, `DisableActionInstruction.cs` + `DisableTarget` variants, `RetryInstruction.cs`, `ClientInstructionEnvelope.cs`, `IClientAccessor.cs`, `NotificationKind` enum.
...
- **Modified**: `MintPlayer.Spark/Services/RetryAccessor.cs` — `Action(...)` now pushes a `RetryInstruction` to `IClientAccessor` before throwing. Exception type stays (`SparkRetryActionException` — reused name).
- **Modified**: every action-endpoint response path (`Endpoints/PersistentObject/{Create,Update,Delete,Get,List}.cs`, `Endpoints/Queries/Execute.cs`, `Endpoints/Actions/ExecuteCustomAction.cs`, `Endpoints/PersistentObject/Edit.cs` if any) — wrap returned value in `ClientInstructionEnvelope`, drain instructions. Old retry-specific JSON-builders in the `catch (SparkRetryActionException ex)` blocks replaced with generic envelope serialization.
- **Modified**: error middleware (or each endpoint's error path) — 4xx/5xx responses also emit the envelope.

**Client side:**
- **New**: `node_packages/ng-spark/client-instructions/src/` — instruction interfaces, dispatcher service, `SPARK_CLIENT_INSTRUCTION_HANDLERS` injection token, built-in handlers for each v1 instruction type.
...
- **Modified**: `node_packages/ng-spark/services/src/spark.service.ts` — responses flow through a single interceptor that unwraps `{ result, instructions }`, dispatches, returns `result` to the caller so consumer code sees the primary payload unchanged.
...
**Demo / E2E:**
- **Modified**: Fleet `CarActions` — add one worked example per instruction type. Ideas: `Notify("Car saved")` on successful save, `Navigate(copy)` on a future `CopyCar` CustomAction, `DisableQueryActions("cars-overview", "Approve")` after approving all items, `RefreshAttribute` on a computed field that only the server knows.
- **New**: Playwright E2E tests covering each instruction type's observable behavior.
...
- [ ] Every action endpoint returns `{ result, instructions }`. Unit-level contract test: `ClientInstructionEnvelopeTests.cs`.
- [ ] All five non-blocking instruction types wired end-to-end with a Fleet demo call site + Playwright test per type.
...
- [ ] Frontend dispatcher silently drops unknown instruction types (unit test pushes `{ type: "futureThing" }` and asserts no throw, no console error).
- [ ] Consumer-defined handlers via `SPARK_CLIENT_INSTRUCTION_HANDLERS` work end-to-end (unit test + one worked example in a demo app).
...
- **Retry subsumption plumbing.** The existing retry flow has subtleties — `retryResults[]` round-trips for state reconstruction, the server's replay relies on deterministic re-execution, and the retry instruction is emitted *during* exception unwind rather than after a clean return. Pin it with a unit test covering "notify + retry in the same exception path" — asserts both instructions in the 449 envelope, in emission order.
- **Instruction ordering.** Instructions execute in array order on the frontend. `Navigate(...)` before `Notify(...)` means the navigation happens first, potentially unmounting the component that would've received the toast. Emission order = execution order; document for developers.
...
2. ~~**Should there be a "Confirm" instruction?**~~ **Resolved: skip.** Retry handles the question-user use case. Adding a lightweight `"confirm"` instruction is speculative convenience until a real friction point emerges. Slim surface area beats pre-emptive ergonomics.
3. ~~**Client-side handler registration syntax.**~~ **Resolved: `multi: true` provider token.** Matches the `SPARK_AUTH_ROUTE_PATHS` pattern already used in `@mintplayer/ng-spark-auth`, so the idiom is consistent across the family. Consumer registration:
   ...
   ```
   The dispatcher injects `SPARK_CLIENT_INSTRUCTION_HANDLERS` (typed as `ClientInstructionHandlerRegistration[]`) plus its built-in map; consumer registrations win on collision.
4. ~~**Envelope on error responses.**~~ **Resolved: yes.** HTTP 400/401/403/500 responses also carry `{ result, instructions }`. A backend validation error can emit `notify` to surface the error without the frontend hand-parsing error bodies. Zero extra server code — the envelope assembly doesn't short-circuit on the error path.
5. ~~**Query-execute envelope.**~~ **Resolved: yes.** Query-execute responses wrap in the same envelope (`{ result: { data, totalRecords, skip, take }, instructions }`). Queries can emit instructions via `CustomQueryArgs`-accessible `IManager` — not as theoretical as it first seemed. Uniform envelope beats per-endpoint carve-outs.
