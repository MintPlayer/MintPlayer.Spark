...
**And an eighth — the authorization model finally has a stated rule, and stating it found two more defects.** The PR spent M9–M11 extending authorization outward (composite scheme → credential handlers → cross-module writes) without ever writing down where that extension stops. Asked the question directly, three investigations produced one rule:

> **Authentication varies by surface. Authorization is `security.json` unless there is no Spark identity to name.**

Two axes, not one — and conflating them is what produces the wrong design. "Modules authenticate by certificate" is true and says nothing about authorization; treating the certificate as the whole answer is exactly the property that made an authenticated module omnipotent, on the write side (**F4**) and, undiscovered until this round, on the read side (**F12** — any module with a valid pinned certificate could have the owner push *any* collection, `SparkUsers` included, into a database it controlled, continuously).

The rule is not "exempt the awkward callers". Where an identity *can* exist it should, and then the write is governed — M11 did not special-case modules to skip authorization, it gave them one (`Module:{Name}`) and ran them through the same path as a person. The exemption applies only where no identity exists and none can be invented: a webhook (GitHub is not a Spark principal) and a background tick.

Testing that against the four authorization configurations is what made it a decision rather than a preference. Gating framework bookkeeping **breaks the deny-all default outright**, **breaks a configured `security.json` silently**, and in the permissive-default case only "works" by falling through to allow-anyone: one of four configurations unaffected, two broken, the fourth no safer. And the mechanism underneath is a genuine trap — a background worker has no `HttpContext`, and the group provider returns an **empty list** rather than throwing, so calling `IPermissionService` there silently evaluates the worker **as anonymous** rather than as a trusted system.

**The lesson worth carrying: an authorization model needs a stated boundary, not only a stated mechanism.** Every extension in M9–M11 was individually defensible, and the absence of a rule about where they stop is what left F12 unnoticed through an entire security audit — the findings document covering replication mTLS never mentioned `/spark/etl/deploy` at all. It also produced **F13**, a defect in M11's own fix: the endpoints validated a caller and never authenticated one, so the writes M11 had just routed through `IPermissionService` would have arrived anonymous and been refused everywhere. Two of this PR's most consequential findings came from asking "where does this stop?" rather than from reviewing any diff.
