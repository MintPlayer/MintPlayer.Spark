...
> **Status: superseded by [`docs/PRD-ClientInstructions.md`](./PRD-ClientInstructions.md).**
> That PRD generalizes this section: instead of just Navigate/Notify, it introduces a unified **Client Instructions** envelope (`{ result, instructions }`) that carries `navigate`, `notify`, `refreshAttribute`, `refreshQuery`, `disableAction`, and `retry` as typed instruction kinds. The API surface lives on `IManager.Client` (sibling to `IManager.Retry`). See the Client Instructions PRD for the full design. The §8 content below is preserved as historical context.
