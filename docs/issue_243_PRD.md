...
- Pre-existing spec-mock naming drift (`canUpdate` vs `canEdit` in two ng-spark spec mocks) —
  cosmetic, fix opportunistically only if those files are already being touched.
