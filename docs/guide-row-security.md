...
| Hook | Purpose | Shape |
|---|---|---|
| `GetRowFilter(action)` | The row rule as an expression the framework **pushes into the RavenDB query**. The primary hook. | `Expression<Func<T,bool>>?` |
...
## `GetRowFilter` — the primary hook

Prefer this. It expresses the rule as an expression, so the framework composes it into the RavenDB query and a list over a row-scoped type reads **only the caller's rows** instead of the whole collection.
...
public class RepositoryActions : DefaultPersistentObjectActions<Repository>
{
    ...
    public override Expression<Func<Repository, bool>>? GetRowFilter(string action)
    {
        ...
        var owners = orgAccess.AllowedOwners;
...
- **Only `GetRowFilter` overridden** → list paths push the expression into the query; single-row checks (detail, edit, delete) compile the same expression. List and detail cannot diverge — they're the same expression.
...
A common pattern: grant a type's `Query`/`Read` right to **`Everyone`** in `security.json`, then use `GetRowFilter` to expose only the public subset. Coverage does exactly this — anonymous viewers see public repositories; authenticated viewers additionally see private repos their identity can access:

```csharp
public override Expression<Func<Repository, bool>>? GetRowFilter(string action)
{
    if (currentUser.IsAdmin) return null;                 // no restriction
    var owners = orgAccess.AllowedOwners;                 // empty for anonymous
    return r => !r.IsPrivate || owners.Contains(r.OwnerLogin);
}
```
