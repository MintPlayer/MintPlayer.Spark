...
- **M3 subscription_worker E2E** — `MessageSubscriptionWorker` driven by real RavenDB subscriptions via `SparkTestDriver` (7: happy path, empty-recipients rollup, NonRetryable handler, MaxAttempts=1 dead-letter, single-pickup retry scheduling, unresolvable MessageType, mixed handler rollup); `SyncActionSubscriptionWorker` with co-located `SparkModulesTest` db + stub `IHttpClientFactory` (5: 200/400/404/500 paths, unknown owner module via `RetryNumerator`).
...
- ⚠️ **CORRECTION (2026-09-24): the premise of this item is false, though the fix it produced was right.** There was no 8.0.1 breaking change (`v8.0.0...v8.0.1` touches only `PublicAPI.*.txt`), and the built-in middleware has never keyed on content type — it filters by HTTP **method** (`IsValidHttpMethodForForm` = POST/PUT/PATCH), which shipped in 8.0.0. **A JSON POST *is* validated by it.** The real reasons Spark ships its own gate: `AntiforgeryMiddleware.InvokeAwaited` records the verdict on `IAntiforgeryValidationFeature` and calls `next` regardless — rejection belongs to whoever binds the form, and an endpoint that binds none has no such consumer — and it skips `DELETE` entirely. This sentence was copied into `SparkMiddleware.cs` and `SparkAntiforgeryMiddleware`'s XML doc and taught the same mistake there for a year; both are now corrected. Original text follows.
...
3. **Reference the generator as `<ProjectReference ReferenceOutputAssembly="false" SkipGetTargetFrameworkProperties="true" />`.** That stops MSBuild from complaining about the netstandard2.0 ↔ net10.0 framework mismatch and stops the generator's compile-time types from leaking into the test project. **Every generator project whose DLL is copied in step 4 needs its own `ProjectReference` like this** — the reference is what establishes the build-graph edge, even though we don't link against the output. `MintPlayer.Spark.SourceGenerators.Tests` references both `MintPlayer.Spark.SourceGenerators` and `MintPlayer.Spark.AllFeatures.SourceGenerators` for this reason.
4. **Copy the runtime-needed DLLs into the test bin via a `<Target AfterTargets="Build">`:** each generator DLL (from `bin/$(Configuration)/netstandard2.0/`) AND `MintPlayer.SourceGenerators.Tools.dll` (from `$(NuGetPackageRoot)mintplayer.sourcegenerators.tools/10.19.0/lib/netstandard2.0/`). `Assembly.LoadFrom` of a generator DLL triggers a Tools load via the probing path. **Do not set `ContinueOnError="true"` on the `<Copy>` task** — a missing generator DLL should fail the build, not degrade into an `MSB3030` warning that's silent until tests crash with `FileNotFoundException` at runtime.
...
> **Past incident (PR #122 → hotfix 72a8d41):** `MintPlayer.Spark.AllFeatures.SourceGenerators` was listed in the Copy target but had no `ProjectReference`, so no build-graph edge existed. MSBuild's parallel build ordering was timing-dependent and had been lucky for months. Adding `coverlet.collector` as an analyzer to `MintPlayer.Spark.SourceGenerators.Tests` in PR #122 shifted build timings enough that the Copy ran before `AllFeatures.SourceGenerators` finished building. `ContinueOnError="true"` swallowed the missing file as a warning; three tests then died at runtime with `FileNotFoundException`. Fix was to add the missing `ProjectReference` and drop `ContinueOnError`.
...
- **Codecov onboarding is required before the token works.** Setting the `CODECOV_TOKEN` secret is necessary but not sufficient. The repo must also exist on Codecov's side: sign in at [app.codecov.io](https://app.codecov.io/login) with GitHub, grant the `MintPlayer` org access (or install the [Codecov GitHub App](https://github.com/apps/codecov) on this repo), then copy the repo-level upload token from the repo's Settings page in Codecov into the `CODECOV_TOKEN` secret. Until the repo is activated, every upload returns `{"message":"Repository not found"}` with HTTP 404. The workflow uses `fail_ci_if_error: false`, so this manifests as a green CI run with no badge data — silent, not a loud failure. If the badge stays empty after a merge, check the "Upload coverage to Codecov" step log for a 404 on the upload request before blaming the workflow.
