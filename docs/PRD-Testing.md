...
- **`SyncActionSubscriptionWorker`'s retry loop runs at subscription-change-vector speed, not at `@refresh` speed.** The subscription query is `from SparkSyncActions where Status = 'Pending'`. When the worker saves the document back with `Status=Pending` (and `@refresh` metadata), the change-vector advance immediately re-triggers delivery — the `@refresh` metadata is never consulted by the subscription. So the `RetryNumerator` burns through `MaxAttempts` (default 5) in milliseconds, not over the configured linear backoff. E2E assertions must target terminal `Status=Failed` (counter deleted at exhaustion), not intermediate counter values — a local run caught it at `counter=1` while CI caught it at `counter=4`.
