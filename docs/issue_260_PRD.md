...
| **R20** | An attribute inside an AsDetail row may trigger a refresh, addressed by the existing `{attr}[{index}].{col}` path convention, without destroying row focus. The refresh runs against the **row's** type — `CarreerJob.ProfessionId` reaches `CarreerJobActions`, not the owning `PersonActions` — with the owner supplied as `args.PersistentObject.Parent`. |
...
| Per-**row** metadata inside a detail grid | Confirmed in the browser: a nested refresh reshapes the *column*, so making `ContractEnd` read-only for one row makes it read-only for every row. Inherent to the grid rendering from one shared `EntityAttributeDefinition` per column rather than a PersistentObject per row (which is how Vidyano gets per-row metadata). Changing that is a rewrite of the inline grid, not a refinement of this feature. Values remain correctly per-row. |
