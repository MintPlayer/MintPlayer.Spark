...
- [ ] **Worked demo example**: add a `ConfirmDeleteCar` Virtual PO to Fleet (new `Demo/Fleet/Fleet/App_Data/Model/ConfirmDeleteCar.json` — follows the parent PRD §8 example shape). Because `EntityTypeDefinition.ClrType` is currently required, Virtual POs also need a no-op marker class (e.g. `Fleet.VirtualObjects.ConfirmDeleteCar`) referenced by the JSON's `clrType` so the model loader can resolve the type. Override `CarActions.OnDeleteAsync` (not `OnBeforeDeleteAsync` — the retry-action loop needs to gate the delete itself, which `OnBeforeDeleteAsync` can't because the outer `DefaultPersistentObjectActions.OnDeleteAsync` continues to `session.Delete` regardless of what the hook does). On `Cancel`, return early (silent NoContent — nothing to delete). On `Delete` with mismatched confirmation, throw an `InvalidOperationException`.
- [ ] **Retry-action round-trip E2E** (API level): `MintPlayer.Spark.E2E.Tests/Retry/RetryActionDeleteTests.cs` drives the full wire loop via `SparkClient.SendAsync` (CSRF + cookies handled) — first DELETE returns 449 with the scaffolded `ConfirmDeleteCar` PO, retry DELETE with `retryResults: [{ step, option, persistentObject }]` body succeeds when the `Confirmation` attribute matches the plate. Three cases: correct confirmation (car gone), Cancel (car survives), wrong confirmation (`>= 400`, car survives). Playwright UI test is deferred — the API test covers the server loop; the modal render is covered by the ng-spark unit tests.
- [ ] **Manual smoke test**: with Fleet running (browser + server), execute the Delete action on a Car, verify the modal renders a LicensePlate (read-only) + Confirmation (required) field with the expected labels. Submit triggers the server-side delete.
