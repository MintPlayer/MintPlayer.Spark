...
{
    // GetCompanies (Company). The anonymous group has QueryRead/Company in App_Data/security.json,
    // so anonymous /execute is authorized — the point of this test is clamping, not authz.
    // (…440002 is GetPeople/Person, which the anonymous group may NOT query — that returned 401.)
