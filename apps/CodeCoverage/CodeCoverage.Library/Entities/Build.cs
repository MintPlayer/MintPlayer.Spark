...
public class Build
{
    ...
    /// <summary>The attempt number of that workflow run.</summary>
    /// <remarks>A re-run of the same run creates a separate build.</remarks>
    public int CiRunAttempt { get; set; }

    /// <summary>The workflow run this build came from, as "runId.attempt".</summary>
    /// <remarks>
    /// Deterministic from <see cref="CiRunId"/> and <see cref="CiRunAttempt"/>, both of which are
    /// fixed at creation.
    /// <para>
    /// Stored rather than computed, which it was until the Builds index became generated: a
    /// generated index maps <c>build.Run</c> straight through, and that runs server-side against
    /// the JSON document — where a get-only CLR property does not exist. It would have indexed as
    /// null on every build, and the grid projects through the index, so the column would have
    /// quietly gone blank. Assign it through <see cref="ComposeRun"/> so the format lives in one
    /// place.
    /// </para>
    /// </remarks>
    ...
    /// <summary>The GitHub Actions workflow that produced this build.</summary>
    public string? WorkflowName { get; set; }

    /// <summary>The GitHub event that triggered the run.</summary>
    /// <remarks>For example <c>push</c> or <c>pull_request</c>.</remarks>
    public string? EventName { get; set; }

    /// <summary>This run measured only a subset of the workspace.</summary>
    /// <remarks>
    /// Declared by the uploader — e.g. <c>nx affected</c> on a pull request. Comparisons must scope
    /// or project rather than read the totals as whole-workspace, and a partial build never becomes
    /// a repository's headline number.
    /// </remarks>
    public bool Partial { get; set; }

    /// <summary>The base sha the uploader's affected-computation ran against.</summary>
    /// <remarks>
    /// What was passed to <c>nx affected --base</c>. Declared, not inferred — deliberately a
    /// dedicated field rather than <see cref="Entities.Commit.ParentSha"/>, which has two writers
    /// and a history of meaning drift and stays a hint.
    /// </remarks>
    public string? DeclaredBaseSha { get; set; }

    /// <summary>The tests behind this run passed, so unmeasured files may be filled in from the base.</summary>
    /// <remarks>
    /// Declared by the uploader. False when any job of the run said its tests failed — a crashed
    /// suite emits no report, and the server cannot tell "affected but crashed" from "unaffected",
    /// so the assembler carries nothing for the commit instead of papering over the crash with the
    /// base's numbers.
    /// </remarks>
    ...
    /// <summary>When the most recent upload arrived.</summary>
    /// <remarks>The debounce finalize counts from here.</remarks>
    public DateTime? LastUploadAtUtc { get; set; }

    /// <summary>When the build was closed and its coverage computed; empty while still open.</summary>
    ...
    /// <summary>Whole-build line and branch totals; empty while the build is open.</summary>
    /// <remarks>Merged across all sessions at finalize.</remarks>
    public CoverageSummary? Coverage { get; set; }

    /// <summary>Added-lines coverage vs the diff base; empty when no diff was obtainable.</summary>
    public PatchCoverage? Patch { get; set; }

    /// <summary>Per-flag totals, keyed by flag name.</summary>
    /// <remarks>
    /// Computed at finalize from the per-flag merged file documents. Null for builds parsed before
    /// flags gained storage — attribution cannot be recovered from the merged build-level
    /// documents.
    /// </remarks>
    public Dictionary<string, CoverageSummary>? FlagCoverage { get; set; }

    /// <summary>Check-run outbox; empty until the first publish attempt is enqueued.</summary>
    ...
    /// <summary>The effective gate this build was judged by.</summary>
    /// <remarks>
    /// The settings document merged with the base ref's coverage.yml. Snapshotted because a
    /// base-dependent verdict is unexplainable later without its inputs.
    /// </remarks>
