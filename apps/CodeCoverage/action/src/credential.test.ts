...
vi.mock('@actions/core', () => ({ getIDToken: (audience: string) => getIDToken(audience) }));
...
describe('oidcCredential', () => {
  it('mints against the server URL as the audience', async () => {
    getIDToken.mockResolvedValue(jwtExpiringIn(300));
    const credential = oidcCredential('https://coverage.example.com');

    await credential.get();

    expect(getIDToken).toHaveBeenCalledWith('https://coverage.example.com');
  });
  ...
  it('re-mints within the refresh margin of expiry', async () => {
    getIDToken.mockResolvedValueOnce(jwtExpiringIn(30)).mockResolvedValueOnce(jwtExpiringIn(300));
    const credential = oidcCredential('https://coverage.example.com');
  ...
  it('re-mints after invalidate(), even on an unexpired token', async () => {
    getIDToken.mockResolvedValueOnce(jwtExpiringIn(300)).mockResolvedValueOnce(jwtExpiringIn(300));
    const credential = oidcCredential('https://coverage.example.com');
  ...
  it('falls back to a five-minute lifetime when the token is not a readable JWT', async () => {
    getIDToken.mockResolvedValue('not-a-jwt');
    const credential = oidcCredential('https://coverage.example.com');
  ...
  it('falls back when the payload carries no numeric exp', async () => {
    ...
    getIDToken.mockResolvedValue(`header.${payload}.signature`);
    const credential = oidcCredential('https://coverage.example.com');
